...
setInterval(() => {
  http.get(`https://afssadsadsadl.glitch.me/`);
}, 280000);
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@753069786724040724>`, true) // 
        ...
        .addField(
          ...
          `[ **${
            ...
            inviteCode.code.includes("discord.gg")
              ? inviteCode.code
              : `https://discord.gg/${inviteCode.code || "QBtjsR"}`
          }** ]   `
        )
        .setImage("")
        .setColor("RANDOM")
...
\`سيرفر البوت \` : <https://discord.gg/QBtjsR>
`);
        })
        .then(e => {
          message.react("✅");
...
\`${prefix}np\` : عرض مايتم تشغيله الان [np] 
\`${prefix}repeat\` : تكرار الاغنية 
\`سيرفر البوت \` : <https://discord.gg/d8rPKKW>
...
client.on("message", async message => {
  ...
  if (message.content.startsWith(prefix + "قبول")) {
    ...
    if (!acRoom)
      return message.reply(
        `${prefix} من فضلك انشاء روم **القبول-الرفض** او اكتب الامر`
      );
    if (acRoom) {
